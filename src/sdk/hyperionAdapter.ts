...
import { scriptComposer, type AptosScriptComposer } from '@moar-market/sdk/composer';
...
export async function executeTransaction(tx: any) {
...
export async function getActiveAPTUSDCPosition(creditAccount: Address, pool: Address) {
  const positions = await getAllPositionsView(creditAccount as any);
  return positions.find((x: any) => x.pool === pool) ?? positions[0] ?? null;
}

export function roundToSpacing(tick: number, spacing = 1) { return Math.round(tick / spacing) * spacing; }
export function pctToHalfWidthTicks(centerTickOrPrice: number, widthPct: number, spacing = 1) {
...
export async function composeRangeExit({
  creditAccount, pool, tokenAPT, tokenUSDC, feeTier,
  lower, upper, fundUSDCu64,
}: {
  creditAccount: Address; pool: Address; tokenAPT: Address; tokenUSDC: Address; feeTier: number;
  lower: number; upper: number; fundUSDCu64: bigint;
}) {
  ...
  if (!p) throw new Error('no hyperion position');
  const info = await get_position_info(p.position_object, p.pool);

  return scriptComposer({
    config,
    sender,
    builder: async (b: AptosScriptComposer) => {
      await removeLiq(b, creditAccount as any, {
        position_v3: p.position_object,
        liquidityDelta: info.liquidity,
        tokenA: tokenAPT as any,
        tokenB: tokenUSDC as any,
        feeTier,
        minAmountA: 0n,
        minAmountB: 0n,
      });
      ...
      await addLiqOpt(b, creditAccount as any, {
        tickLower: lower,
        tickUpper: upper,
        tokenA: tokenAPT as any,   // tokenA = APT (8dp)
        tokenB: tokenUSDC as any,  // tokenB = USDC (6dp)
        feeTier,
