...
export function getConfig(): RuntimeConfig { return cached; }
export function setConfig(next: RuntimeConfig) { cached = schema.parse(next); fs.writeFileSync(CONFIG_PATH, JSON.stringify(cached, null, 2)); }

fs.watchFile(CONFIG_PATH, { interval: 1000 }, () => { try { cached = load(); } catch { /* ignore */ } });
