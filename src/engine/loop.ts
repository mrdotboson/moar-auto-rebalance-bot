...
export async function step(cfg: RuntimeConfig) {
  if (Date.now() < cooldownUntil) return { ok: true, action: 'cooldown' };

  const eps = 1; // hysteresis (tickSpacing=1 for feeTier=1)
  const { tick } = await getCurrentTickAndSqrt(cfg.pool);

  const positions = await getAllPositionsView(cfg.creditAccount as any);
  const p = positions.find((x: any) => x.pool === cfg.pool) ?? positions[0];
  ...
  if (!outside) return { ok: true, action: 'idle' };
